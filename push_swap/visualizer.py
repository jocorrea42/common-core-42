...
class PushSwapVisualizer(Gtk.Window):
    ...
    def draw_ps(self, area: Gtk.DrawingArea, context: cairo.Context) -> None:
        ...
        for i, v in enumerate(self.stack_a):
            ...
            context.rectangle(0, i * height, v_percent * wsize.width, height)
            context.fill()

        # Draw Separation line
        context.set_source_rgb(0.3, 0.3, 0.3)
        context.rectangle(0, wsize.height / 2, wsize.width, self.SEP_HEIGHT)
        context.fill()
        ...
        for i, v in enumerate(self.stack_b):
            ...
            context.rectangle(
                0,
                wsize.height / 2 + self.SEP_HEIGHT + i * height,
                v_percent * wsize.width,
                height,
            )
            context.fill()
